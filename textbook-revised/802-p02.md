...
The `PickerState` interface and `getDefaultPickerState` function are defined in `src/app/framework/models/picker-config.interface.ts` alongside other picker configuration types:

```typescript
// src/app/framework/models/picker-config.interface.ts
// Picker state and configuration interfaces
...
Note: The `PickerState` interface is co-located in the `picker-config.interface.ts` model file alongside `PickerConfig`, `PickerApiParams`, and `PickerSelectionEvent` interfaces, rather than in a separate component-local file.
