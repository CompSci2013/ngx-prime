...
**Figure 306.1** shows the complete data flow from user action through to UI update:

![Complete data flow from user action to UI update](../docs/diagrams/state-management-specification-flow-1-user-updates-filter.png)

*Figure 306.1: The URL-First data flow. When a user clicks a filter, the URL updates, which triggers ResourceManagementService to fetch data. All subscribed components automatically receive the updated state.*
