...
export class BasicResultsTableComponent<TFilters = any, TData = any, TStatistics = any>
   ...
   * Handle sort change
   *
   * Note: Includes debug logging for development/troubleshooting pop-out scenarios.
   */
...
Note: This component uses synchronous getters (not Observable streams) for accessing state from `ResourceManagementService`. This simplifies template bindings and avoids unnecessary async pipe usage. The `DynamicResultsTableComponent` variant uses Observable streams with async pipe for scenarios requiring OnPush change detection optimization.
