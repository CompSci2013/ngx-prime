...
│                    │           │    │                               │
│                    ▼           │    │                               │
│  fetchData(A):     ●───────X   │    │   (request cancelled)         │
│                            ▲   │    │                               │
│                            │   ▼    │                               │
│  fetchData(B):             │   ●──X │   (request cancelled)         │
│                            │      ▲ │                               │
│                    cancel ─┘      │ ▼                               │
│  fetchData(C):              cancel─┘ ●────────Y──►                  │
│                                               │                     │
│  results$:        ────────────────────────────Y──►                  │
│                                                                     │
│  Legend: ● = request starts, X = cancelled, Y = result emitted      │
└─────────────────────────────────────────────────────────────────────┘
```

*Figure 250.2: switchMap cancels in-flight requests when new values arrive, ensuring only the latest result is emitted.*
