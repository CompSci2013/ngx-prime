...
Update `src/app/domain-config/automobile/adapters/index.ts`:

```typescript
// src/app/domain-config/automobile/adapters/index.ts
...
```bash
$ ls -la src/app/domain-config/automobile/adapters/
```
...
```bash
$ npx tsc --noEmit src/app/domain-config/automobile/adapters/index.ts
```

### 3. Test API Adapter

```typescript
import { AutomobileApiAdapter } from '@app/domain-config/automobile/adapters';
import { ApiService } from '@app/framework/services';
import { AutoSearchFilters } from '@app/domain-config/automobile/models';
...
```typescript
import { AutomobileCacheKeyBuilder } from '@app/domain-config/automobile/adapters';
import { AutoSearchFilters } from '@app/domain-config/automobile/models';
...
// → 'auto:h_yearMax=2024:h_yearMin=2020:manufacturer=Toyota'
