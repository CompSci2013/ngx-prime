...
@NgModule({
  ...
  imports: [
    ...
    DragDropModule,         // CDK drag-drop for panel reordering
    AppRoutingModule,
    PrimengModule,          // UI component library
    FrameworkModule         // Core framework components
  ],
  providers: [
...
| Section | Purpose |
|---------|---------|
| `declarations` | All feature components (eagerly loaded, not using separate feature modules) |
| `imports` | Angular core modules plus framework and UI modules |
...
| Module | Purpose |
|--------|---------|
| `FormsModule` | Required for `ngModel` binding in forms |
| `DragDropModule` | CDK drag-drop for panel reordering in DiscoverComponent |
| `PrimengModule` | UI component library (buttons, tables, tooltips, etc.) |
| `FrameworkModule` | Core framework components (QueryControl, BasePicker, etc.) |
...
| `GlobalErrorHandler` | Catches unhandled JavaScript errors |
| `HttpErrorInterceptor` | Catches HTTP errors for consistent handling |
| `DOMAIN_CONFIG` | Provides automobile domain configuration via factory |
