...
**File**: `src/app/domain-config/automobile/index.ts` (exports from `automobile.domain-config.ts`)
...
The domain configuration is composed from multiple files in the `domain-config/automobile/` directory:
- `automobile.domain-config.ts` - Factory function and main config
- `models/` - Type definitions (filters, data, statistics)
- `adapters/` - API, URL, and cache adapters
- `configs/` - Table, filter, chart, and picker configurations
- `chart-sources/` - Chart data source implementations
