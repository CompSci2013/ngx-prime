...
Note: The `ResourceManagementService` is injected with `@Optional()` decorator since the component can work with or without it. When available, the component uses `filters$` for reactive URL synchronization. When not available, it falls back to direct URL watching via `UrlStateService`.
