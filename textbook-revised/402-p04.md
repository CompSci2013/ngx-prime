...
Update `src/app/domain-config/automobile/models/index.ts`:
...
```bash
$ ls -la src/app/domain-config/automobile/models/automobile.data.ts
```

### 2. TypeScript Compilation Check

```bash
$ npx tsc --noEmit src/app/domain-config/automobile/models/automobile.data.ts
```
...
import { VehicleResult, VinInstance } from '@app/domain-config/automobile/models';
