...
**Figure 303.1** illustrates the three-layer request processing model:

![Three-layer request processing: cache, dedup, HTTP](../docs/diagrams/state-management-specification-three-layer-request-processing.png)

*Figure 303.1: The three processing layers. Each request first checks the cache, then checks for in-flight duplicates, and finally executes HTTP with retry logic. This dramatically reduces redundant API calls.*
