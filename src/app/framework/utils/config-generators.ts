 ...
 * Config Generators
 *
 * Utility functions to generate TableConfig and FilterDefinitions
...
/**
 * Generate FilterDefinitions from ResourceDefinition
 *
 * Creates an array of FilterDefinition objects for the query panel
 ...
 *
 * @param definition - ResourceDefinition to generate from
 * @returns Array of FilterDefinitions for query panel
 *
 * @example
 * ```typescript
 * const filterDefs = generateFilterDefinitions(AUTOMOBILE_RESOURCE);
...
/**
 * Generate FilterDefinition for a regular (non-range) field
 */
function generateFieldFilterDef(field: ResourceField): FilterDefinition {
...
/**
 * Generate FilterDefinition for a range field group (min/max pair)
 */
...
/**
 * Convert ResourceField type to FilterType
 */
function getFilterType(field: ResourceField): FilterType {
 ...
 *
 * Creates FilterDefinitions for fields marked as highlightable.
 * These are used in the Query Control component for h_* URL parameters.
 *
 * @param definition - ResourceDefinition to generate from
 * @returns Array of FilterDefinitions for highlight controls
 */
export function generateHighlightFilterDefinitions(
  definition: ResourceDefinition
): FilterDefinition[] {
  const highlightDefs: FilterDefinition[] = [];
  const processedRangeFields = new Set<string>();
  const rangeGroups = groupRangeFields(definition);
